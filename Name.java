...
public class Name {
	...
	private static String validateName(String name) throws IllegalArgumentException {
		...
		for (int i = length - 1; i >= 0; i--) {
			...
			// if char is not a letter nor a legal character, throw an exception
			...
			if (name.charAt(i) > 0 && name.charAt(i) < length-1)
				// if the char is a legal character (but not a letter), check to
			...
			// if the length is 2, then an exception will be thrown if
			// one of the two characters are a legal character (but not a letter)
			if (length == 2)
				if (!isLegalChar(c) || !isLegalChar(name.charAt(i - 1)))
					throw new IllegalArgumentException(c + " must be in between two letters");
		}
		return name;
	}
